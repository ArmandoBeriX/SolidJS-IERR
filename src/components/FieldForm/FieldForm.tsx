...
const FieldForm = (props: FieldFormProps) => {
  ...
  createEffect(() => {
    ...
    if (isEditing() && props.field) {
      ...
      // Procesar min/max - Asegurarse de que se carguen como strings para los inputs
      let minValue = '';
      let maxValue = '';
      
      if (existingStoreData.min !== undefined && existingStoreData.min !== null) {
        minValue = String(existingStoreData.min);
      }
      
      if (existingStoreData.max !== undefined && existingStoreData.max !== null) {
  ...
  const handleSubmit = (e: Event) => {
    ...
    // Preparar datos para enviar - AQUÍ ES LA CLAVE: Asegurar que storeData se procese correctamente
    const submitData = {
      ...
      storeData: {
        // Procesar min y max: convertir a número solo si tienen valor
        min: formData().storeData.min !== '' && formData().storeData.min !== null ? 
             (isNaN(Number(formData().storeData.min)) ? formData().storeData.min : Number(formData().storeData.min)) : '',
        max: formData().storeData.max !== '' && formData().storeData.max !== null ? 
             (isNaN(Number(formData().storeData.max)) ? formData().storeData.max : Number(formData().storeData.max)) : '',
        ...
        // Solo incluir possibleValues si es un campo de tipo list
    ...
    console.log('✅ Datos procesados para guardar:', JSON.stringify(submitData, null, 2));
