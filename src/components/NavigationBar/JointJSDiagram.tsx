...
const JointJSDiagram = (props: JointJSDiagramProps) => {
  ...
  // SISTEMA MEJORADO DE DETECCIÓN DE CLICKS - INCLUYE BOTÓN AGREGAR CAMPO
  const handleElementClick = (cellView: joint.dia.CellView, evt: joint.dia.Event) => {
    ...
    if (dataAction === 'add-field') {
      console.log('➕ NUEVO: Botón agregar campo clickeado en:', {
        tableId: dataTableId,
  ...
  // SISTEMA MEJORADO DE CREACIÓN DE ENTIDADES - CON BOTÓN AGREGAR CAMPO FUNCIONAL
  const createEntityElement = (table: TableData) => {
    ...
    // ÁREA CLICKEABLE DEL BOTÓN - CRÍTICO: Agregar atributos data-*
    ...
    // Agregar todos los elementos al markup - CRÍTICO: Agregar atributos data-* al área clickeable
    const markup = entity.markup as any[];
    markup.push(
  ...
  const initializeJointJS = () => {
    ...
    try {
      ...
      // Configurar eventos - CRÍTICO: Detectar clics en el botón agregar campo
