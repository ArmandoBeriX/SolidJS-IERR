...
const NavigationBar = () => {
  ...
  // NUEVO: Estado para controlar si estamos arrastrando
  const [isDragging, setIsDragging] = createSignal(false);
  const [dragStartTime, setDragStartTime] = createSignal(0);

  // NUEVO: Efecto MEJORADO para escuchar eventos de clic en nombres de campo
  createEffect(() => {
    const handleFieldNameClick = (event: CustomEvent) => {
      console.log('📢 Evento fieldNameClick recibido en NavigationBar - DATOS COMPLETOS:', event.detail);
      const { tableId, field, tableName } = event.detail;
      
      // VERIFICACIÓN MEJORADA de datos
      console.log('🔍 Verificando datos del campo:', {
        tieneTableId: !!tableId,
        tieneField: !!field,
        tieneTableName: !!tableName,
        fieldId: field?.id,
      ...
      if (tableId && field && tableName) {
        console.log('✅ Abriendo modal para editar campo:', {
          fieldName: field.name,
          fieldId: field.id,
          fieldFormat: field.fieldFormat,
          tableName: tableName,
        ...
        // Buscar los datos ACTUALIZADOS desde localStorage o tablesData
        ...
        if (table) {
          ...
          const actualField = table.tableFields?.find(f => f.id === field.id);
          
          if (actualField) {
            console.log('✅ Campo actual encontrado:', actualField);
            console.log('📋 Datos completos del campo:', {
              nombre: actualField.name,
              identificador: actualField.identifier,
              formato: actualField.fieldFormat,
              requerido: actualField.isRequired,
              descripcion: actualField.description,
              esPK: actualField.identifier === 'id',
              esRelacion: actualField.fieldFormat === 'relation'
            });
            
            openFieldModalWithData(tableId, actualField, tableName);
            return;
          } else {
            console.log('⚠️ Campo no encontrado por ID, buscando por nombre...');
            // Buscar por nombre como fallback
            const fieldByName = table.tableFields?.find(f => 
              f.name === field.name || f.identifier === field.identifier
            );
            if (fieldByName) {
              console.log('✅ Campo encontrado por nombre:', fieldByName);
              openFieldModalWithData(tableId, fieldByName, tableName);
              return;
            }
          }
        } else {
          console.log('❌ Tabla no encontrada en tablesData');
        }
        
        // Si no se encuentra, usar el campo del evento
        console.log('⚠️ Usando campo del evento (no encontrado en datos actuales)');
        openFieldModalWithData(tableId, field, tableName);
      } else {
        console.error('❌ Datos incompletos en evento fieldNameClick:', event.detail);
      }
    };

    // Usar capture phase para asegurar que se reciba el evento
    document.addEventListener('fieldNameClick', handleFieldNameClick as EventListener, true);
    
    onCleanup(() => {
      document.removeEventListener('fieldNameClick', handleFieldNameClick as EventListener, true);
    });
  });

  // NUEVO: Función para abrir modal de campo con datos verificados
  ...
  const openFieldModal = (tableId: number, field?: TableField, tableName?: string) => {
    ...
    // NUEVO: Verificar datos antes de abrir el modal
    if (field) {
      console.log('🔍 Verificando datos del campo antes de abrir modal:', {
        id: field.id,
        name: field.name,
        identifier: field.identifier,
        fieldFormat: field.fieldFormat,
  ...
  // NUEVO: Función para manejar inicio de arrastre
  ...
  // NUEVO: Función para manejar fin de arrastre
  const handleDragEnd = () => {
    setIsDragging(false);
    console.log('🔄 Finalizando arrastre...');
  };

  // Función para manejar clic en elementos del diagrama - MODIFICADA
  const handleElementClick = (cellView: joint.dia.CellView, evt: joint.dia.Event) => {
    ...
    // Click en el header - SOLO si no es arrastre
    if ((selector === 'header' || className.includes('header'))) {
      // Verificar que no sea un arrastre (click prolongado)
      const clickDuration = Date.now() - dragStartTime();
      if (clickDuration < 200) { // Si fue un click rápido (menos de 200ms)
    ...
    // Los clicks en nombres de campo ahora se manejan mediante el evento personalizado
